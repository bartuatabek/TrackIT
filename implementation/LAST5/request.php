...
function update_project($db,$name,$description, $status, $start_date,$end_date,$project_id)
{
    ...
    $sql=mysqli_query
        ($db,
         "
            UPDATE Project SET status = '$status', name = '$name',start_date ='$start_date', end_date='$end_date',description='$description' WHERE project_id=$project_id;
		"
        );
}
